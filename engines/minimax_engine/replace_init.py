...
class BaseBoard:
    ...
    def zobrist_hash(self):
        ...
        for index, square in enumerate(SQUARES_180):
            piece = self.piece_at(square)
            if piece:
                h ^= self.zobrist_map[int(index / 8)][int(index % 8)][int(piece.__hash__())]
        return h
...
class Board(BaseBoard):
    ...
    manipulation.
    """
    zobrist_map = [[[random.randint(1,2**64 - 1) for i in range(12)]for j in range(8)]for k in range(8)]
